...
const ProductReportChart = () => {
  ...
  const {
    ...
    selectedTab && currentYear
      ? `/products/stats/report?period=${apiQueryPeriodMap[selectedTab]}&year=${currentYear}`
      : null,
    fetcher,
  ...
  const prepareExportData = () => {
    ...
    return {
      pdf: {
        ...
        fileName: "Laporan_Produk_Mobil.pdf",
        periodeText,
      },
      csv: {
        headers: [
        ...
        data: csvData,
        fileName: "Laporan_Produk_Mobil.csv",
      },
    };
  };
      ...
      >
        ...
        >
          <div className="flex items-start gap-3 p-3 border border-gray-200 rounded-lg bg-gray-50 h-full">
            <div className="flex-shrink-0 flex items-center justify-center w-10 h-10 bg-sky-100 rounded-lg">
              <FaCar className="w-5 h-5 text-sky-500" />
            </div>
            <div>
