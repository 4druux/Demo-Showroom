...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUserProfile = useCallback(async (token) => {
    ...
    try {
      ...
      localStorage.removeItem("demoShowroomAuthToken");
      setUser(null);
      const message =
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (data.otpRequired) {
        ...
        localStorage.setItem("demoShowroomAuthToken", data.token);
  ...
  const handleOAuthSuccess = useCallback((oauthData) => {
    localStorage.setItem("demoShowroomAuthToken", oauthData.token);
